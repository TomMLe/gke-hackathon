...
@click.option('--port', 'port', default=10103)
@click.option('--agent-card', 'agent_card', default='recommend_agent/card.json')
def main(host, port, agent_card):
    """Starts the Recommend Agent as an A2A server."""
    try:
        ...
        logger.info(f'Starting Recommend Agent server on {host}:{port}')
