    ...
    return AgentCard(
        ...
        skills=[
            AgentSkill(
                ...
                description="Suggests similar items based on abandoned cart",
                tags=[
...
async def _ensure_runner() -> Runner:
    ...
    logger.info(f"MCPToolset initialized {MCP_SSE_URL}")
    ...
    agent = LlmAgent(
        ...
        instruction=(
            ...
            "Propose product recommendations based on the product ID. ALWAYS Use the MCP tools given to you"
    ...
    _runner = Runner(app_name="recommend_agent", agent=agent, session_service=session_service)
    # Ensure the default session exists (works with sync OR async session services)
    await session_service.create_session(
        app_name="recommend_agent",
...
async def a2a_endpoint(payload: A2ARequest):
    ...
    try:
        ...
        if callable(events):
            events = r.run_async(
                user_id="host_orchestrator",
                session_id="recommend_agent_session",  # or recommend_agent_session
                new_message=content,
            )
        else:
            events = r.run(
